...
ContextVault versions the Python product, SQLite schema, client protocol, and each adapter independently. The current set is product `0.12.0`, schema `9`, protocol `3`, and Chrome extension `0.5.0`; the minimum compatible client protocol is `2`. The standalone extension vault uses schema `1`.
...
- protocol-2 clients remain usable inside the current compatibility window; clients below the minimum must upgrade, while clients newer than the server require a server upgrade.
